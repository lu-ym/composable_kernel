...
inline std::string get_type_name()
{
    if constexpr(std::is_same_v<T, half_t>)
        ...
        return "int4";
    else if constexpr(std::is_same_v<T, f4_t>)
        return "f4";
    else if constexpr(std::is_same_v<T, f6_t>)
        return "f6";
    else if constexpr(std::is_same_v<T, bf6_t>)
        return "bf6";
    else if constexpr(std::is_same_v<T, f8_t>)
        return "f8";
    else if constexpr(std::is_same_v<T, bf8_t>)
        return "bf8";
    else if constexpr(std::is_same_v<T, e8m0_bexp_t>)
        return "e8m0";
    else
        return typeid(T).name(); // default data type name
}
